...
def test_infer_irrelevant_middle() -> None:
    by_solver = {
        "baseline": {"selected_indices": "[0, 1, 2, 3, 4]"},
        "llm_dynamic": {"selected_indices": "[0, 1, 3, 4]"},
        "llm_static": {"selected_indices": "[0, 1, 3, 4]"},
    }
...
def test_append_block_notes_writes_section() -> None:
    csv_content = (
        ...
        "example_instance.json,baseline,5,5,45,45,1.0,1.0,1.0,5.0,0.48,0.0,1.8,\"[0, 1, 2, 3, 4]\"\n"
        "example_instance.json,llm_dynamic,5,4,45,38,0.844,0.8,0.844,2.55,0.6,0.2,1.95,\"[0, 1, 2, 4]\"\n"
    )
